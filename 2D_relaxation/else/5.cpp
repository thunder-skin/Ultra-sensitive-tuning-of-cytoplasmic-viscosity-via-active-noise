...
//信息控件
double heat_tense_times=-2.5;
std::string filename="1025";
std::string testcode="0";

//定义工作
bool diagnose=true;
bool error_dealing=false;

//定义x和y方向格数，总步数计数
int x_num=0;
int y_num=0;
int count=0;
double heat_tense=pow(10,heat_tense_times*0.5);
...
std::vector<double> time_list={1,2,3,5,7,
                            10,15,20,30,50,70,
...
std::vector<double> move_to(double fx,double fy,Particle part_self,float t_step){
    double rand_num_x = normal_dist(gen)*pow(t_step,-0.5)*heat_tense;
    double rand_num_y = normal_dist(gen)*pow(t_step,-0.5)*heat_tense;
    double x_move=(fx+rand_num_x)/eta*t_step;
    double y_move=(fy+rand_num_y)/eta*t_step;
    double y_final=0;
    double x_final=0;
...
void write_world(int time_step){
    std::ofstream outFile(filename+"_"+testcode+"_data.txt");
...
int main(){
    ...
    outFile.close();
    std::cout<<" "<<filename<<" "<<heat_tense<<std::endl;
    double L_0=0;
    L_0=read_world();
    sheer+=amp*y_tot;
