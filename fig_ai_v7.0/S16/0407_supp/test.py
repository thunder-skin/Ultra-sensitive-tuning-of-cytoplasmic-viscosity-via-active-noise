...
# 计算B = exp(A)
B_samples = np.exp(2.31*(A_samples-1))
C_samples = 0.1+0.9*A_samples
D_samples = np.log(1.105+(2.718-1.105)*A_samples)

E_samples = np.concatenate((B_samples[:num_samples//2], C_samples[num_samples//2:]))


hist, bin_edges = np.histogram(E_samples, bins=500, density=True)
...
plt.yticks(np.arange(0, 3, 0.5),fontsize=12)
plt.grid(True)
plt.show()
print(min(B_samples))