...
d=plot(data(:,1)*xx,data(:,3)*yy1,"o","Marker","x","MarkerSize",15,"MarkerFaceColor",color(2,:),"Color",color(2,:),"LineWidth",2); 
data=xlsread("data_7.xlsx")
e=plot(data(:,1)*xx,data(:,3)*yy2,"o","Marker","x","MarkerSize",15,"MarkerFaceColor",color(5,:),"Color",color(5,:),"LineWidth",2);
...
y_test=log10(data(:,3)*yy1);
p = polyfit(x_test, y_test, nnn);
x_i=linspace(-0.45,3.8,40)
y_fit = polyval(p, x_i)
f=plot(10.^x_i,10.^y_fit,"--","LineWidth",2.5,"Color",color(3,:)); 

data=xlsread("data_7.xlsx")
x_test=log10(data(:,1)*xx);
y_test=log10(data(:,3)*yy2);
p = polyfit(x_test, y_test, nnn);
x_i=linspace(-0.35,3.8,40)
y_fit = polyval(p, x_i)
...
l=legend([b,a,c,d,e], ...
    'ATP-depleted Cell (Ebata $$\textit{et al.}$$)','Live Cell (Ebata $$\textit{et al.}$$)','Live Cell (Muenker $$\textit{et al.}$$)','$$k_BT_{\rm{eff}}=1.0 \times 10^{-4}$$ (Simulation)','$$k_BT_{\rm{eff}}=4.0 \times 10^{-4}$$ (Simulation)','Interpreter', 'latex');
set(l, 'box', 'off')
